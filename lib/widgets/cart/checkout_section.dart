...
class _CheckoutSectionState extends ConsumerState<CheckoutSection> {
  ...
  Widget _buildCheckoutButton(
    ...
    final isDisabled = checkoutState.isLoading ||
        (_lastOrderTime != null &&
            DateTime.now().difference(_lastOrderTime!) <
    ...
    return SizedBox(
      ...
      child: FilledButton(
        ...
        onPressed:
            isDisabled ? null : () => _showCheckoutDialog(context, ref, total),
        child: Row(
  ...
  void _showCheckoutDialog(BuildContext context, WidgetRef ref, double total) {
    // Reinicia o estado do checkout ao abrir o diálogo
          ...
          content: Column(
            ...
            children: [
              ...
              if (checkoutState.paymentMethod == PaymentMethod.cash)
                Padding(
                  padding: const EdgeInsets.only(
                      top: AppSizes.paddingSmall, left: AppSizes.paddingLarge),
                  child: InfoLabel(
                    label: 'Troco para:',
                    child: NumberBox<double>(
                      value: checkoutState.changeFor,
                      onChanged: (value) {
                        ref
                            .read(checkoutProvider.notifier)
                            .updateChangeFor(value);
                      },
              ...
              _buildDialogPaymentOption(
                ...
                checkoutState.paymentMethod == PaymentMethod.credit,
                (value) {
                  ref
                      .read(checkoutProvider.notifier)
                      .updatePaymentMethod(PaymentMethod.credit);
                },
              ),
              ...
              _buildDialogPaymentOption(
                ...
                FluentIcons.code,
                AppColors.primaryAccent,
                checkoutState.paymentMethod == PaymentMethod.pix,
                (value) {
                  ref
                      .read(checkoutProvider.notifier)
                      .updatePaymentMethod(PaymentMethod.pix);
                },
              ),
            ],
          ),
  ...
  void _confirmOrder(BuildContext context, WidgetRef ref) async {
    final success = await ref
        .read(checkoutProvider.notifier)
        .processCheckout(widget.currentCart);

    final checkoutState = ref.read(checkoutProvider);

    if (success && checkoutState.completedOrder != null) {
      final order = checkoutState.completedOrder!;

      // Gerar e exibir prévia interna do cupom fiscal automaticamente
      if (context.mounted) {
      ...
      if (context.mounted) {
        ...
        showCustomToast(
          ...
          message:
              'Pedido #${order.id.substring(order.id.length - 8)} criado e cupom fiscal gerado!',
      ...
      if (context.mounted) {
        // Se houve erro de validação (ex: carrinho vazio) que não fechou o dialog, ou erro no processamento
        final errorMessage = checkoutState.error ??
